...
def showEntity(request, id):
    ...
    linksfrom = Link.objects.filter(entity1=e)
    linksto = Link.objects.filter(entity2=e)
...
def addLinkForm(request, id):
    ...
    alle = Entity.objects.all().order_by('category','sortkey')
    context = { 'e': e, 'categories': categories, 'allentities': alle }
